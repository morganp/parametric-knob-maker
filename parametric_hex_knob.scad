// TODO Issue base bump is scaling with KNOB_HEIGHT it should have seperate param.
// Chamfers should be constant angles not scaling with knob height
...
translate( [0,0,((KNOB_HEIGHT*2 -SCREWHEAD_DEPTH) /2 + KNOB_HEIGHT/4) ] ) {
//translate( [0,0,(((KNOB_HEIGHT*2 -SCREWHEAD_DEPTH) /2)  OFFSET_HEIGHT) ] ) {
  difference(){
      ...
      union(){
          ...
          translate([0, 0, -(((KNOB_HEIGHT-SCREWHEAD_DEPTH+KNOB_HEIGHT)/2))]) {
              ...
              cylinder(r=KNOB_DIAM/4, h=OFFSET_HEIGHT, center=true,$fn=64);
          //translate([0, 0, 0]) {
          //    cylinder(r=KNOB_DIAM/4, h=KNOB_HEIGHT/2, center=false,$fn=64);
          }
      ...
      ////down filet
      // TODO THIS is hitting the offset.
      translate([0, 0, -(((KNOB_HEIGHT-SCREWHEAD_DEPTH)/2)+0.1)])
        rotate_extrude()
          polygon(points = [[-KNOB_DIAM,-KNOB_HEIGHT], [0,-KNOB_HEIGHT-2],[0,-KNOB_HEIGHT],[-KNOB_DIAM/1.29,0]],$fn=100);
